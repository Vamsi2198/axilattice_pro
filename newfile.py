"""
AxiLattice Pro — Bulletproof Production Version v3
Fixed: Cube init (avg->mean, cnt->size), BOOL->CAT mapping, temp_col overwrite, 
       TTS loop, Voice auto-stop, Insights not showing, Observability
"""
...
# ============================================================
# VOICE I/O — FIXED: NO AUTOPLAY LOOP, PROPER STOP
# ============================================================
...
class VoiceManager:
    ...
    def html(self, audio_bytes: bytes, autoplay=False) -> str:
        b64 = base64.b64encode(audio_bytes).decode()
        auto = "autoplay" if autoplay else ""
        return f'<audio controls {auto} src="data:audio/mp3;base64,{b64}"></audio>'
...
# ============================================================
# DATA PROFILER — FIXED: BOOL->CAT, temp_col overwrite
# ============================================================
...
class DataProfiler:
    ...
    def _run(self):
        for c in self.df.columns:
            ...
            if p.type == ColType.TEMP:
                # FIXED: Don't overwrite temp_col if already set (prefer first temporal)
            ...
            elif p.type == ColType.BOOL:
                # FIXED: Treat boolean as categorical for analytics purposes
                self.cat_cols.append(c)
    ...
    def _infer(self, s, dt, ratio, uniq):
        ...
        if uniq == 2:
            # Keep as BOOL internally but will be treated as CAT in _run()
...
# ============================================================
# STREAMLIT UI — BULLETPROOF v3
# FIXED: TTS loop, voice autoplay, insights display, empty states
# ============================================================
...
for key, default in [
    ...
    ('tts_played', set())  # FIXED: Track which TTS messages were already played
]:
...
with st.sidebar:
    ...
    st.caption("Live Voice | Multi-Dimensional | Bulletproof v3")
...
# ============================================================
# ASK MODE — FIXED: TTS LOOP, VOICE FLOW, INSIGHTS DISPLAY
# ============================================================
if mode and "Ask" in mode:
    ...
    with c1:
        ...
        if st.session_state.last_tts:
            ...
            if st.button("Play Again", key="play_again"):
                ...
                if audio_bytes:
                    st.markdown(voice.html(audio_bytes, autoplay=True), unsafe_allow_html=True)
                else:
    ...
    with c2:
        # FIXED: Display chat history with proper TTS tracking
        for i, m in enumerate(st.session_state.chat[-12:]):
            msg_key = m.get('id', f"msg_{i}")
            if m['role'] == 'u':
                ...
                # FIXED: Only play TTS once per message using tts_played set
                if m.get('tts') and msg_key not in st.session_state.tts_played:
                    audio_bytes = voice.speak(m['tts'])
                    if audio_bytes:
                        st.markdown(voice.html(audio_bytes, autoplay=True), unsafe_allow_html=True)
                    st.session_state.tts_played.add(msg_key)
            ...
            with st.spinner("Analyzing..."):
                try:
                    ...
                    if intent == "trend":
                        ...
                        for ri in results:
                            tts_text = re.sub(r'[\*#]', '', ri.text).replace('\n', ' ')[:500]
                            msg_id = hashlib.md5(f"auto_{tts_text}_{time.time()}".encode()).hexdigest()[:12]
                            st.session_state.chat.append({
                                "role": "a", "text": ri.text, "viz": ri.viz, "data": ri.data,
                                "tts": tts_text, "id": msg_id
                            })
                    ...
                    ms = int((time.time() - t0) * 1000)
                    msg_id = obs.log(q, r['intent'], intent, result, ms)
                    ...
                    st.session_state.chat.append({
                        ...
                        "id": hashlib.md5(f"err_{time.time()}".encode()).hexdigest()[:12]
                    })
                    st.rerun()
...
st.divider()
st.caption("AxiLattice Pro v3 | Live Voice | Multi-Dimensional | 21 Insight Types | Bulletproof")